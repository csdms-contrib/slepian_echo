...
% loris4([],[],[],[],[],1) % Makes a graph of topography after running LORIS2
% 
% actprint=0;
...
% Reviewed by gyin-at-princeton.edu, 07/14/2016
% Last modified by fjsimons-at-alum.mit.edu, 07/14/2016
...
if ~isstr(depkm)
  ...
  if xver==0
    ...
    defval('L',ceil(2^(N+1)))
    % This is where the file is kep
    dirname=fullfile(getenv('IFILES'),'EARTHMODELS','CONSTANTS'); 
  ...
  if doneven==0
    ...
    % Do the printing to EPS... NEEDS UPDATE TO OPENGL
    popts='-zbuffer'',''-r300';
    popts=[];
  ...
  if agu==1
    ...
    %set([ha(1:2)],'CameraViewAngle',3)
    set([ha([1 4])],'CameraViewAngle',3)
    movev(cb,-.05)
