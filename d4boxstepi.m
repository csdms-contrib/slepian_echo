function x=d4boxstepi(f,level,dim,cofs)
% x=d4boxstepi(f,level,dim,cofs)
%
% Performs one INVERSE iteration (from a certain level) of D4 transform
...
if k-3<3; error('Input signal is not long enough'); end
for i=3:k-3
  ...
  % Watch the delay, which is different in D6BOXSTEPI
  % These indices determine the ranges above!
  x(dindeks(2*i+0,dim,nall))=...
      cofs.F0([1 3])*fplaneslo+cofs.F1([1 3])*fplaneshi;
  x(dindeks(2*i+1,dim,nall))=...
      cofs.F0([2 4])*fplaneslo+cofs.F1([2 4])*fplaneshi;
end

% Isolate the indices of the right sets of planes
frightlo=[f(dindeks(  k-2,dim,nall))'; ...
	  f(dindeks(  k-1,dim,nall))'; ...
	  f(dindeks(  k-0,dim,nall))'];
frighthi=[f(dindeks(2*k-2,dim,nall))'; ...
