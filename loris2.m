function loris2(N,L,colmap,meth,fs,opt)
% LORIS2(N,L,colmap,meth,fs,opt)
%
...
set([ttx tty],'color','r')
set(tty,'VerticalAlignment','bottom')
set(ttx,'HorizontalAlignment','left')
set(ttx,'VerticalAlignment','mid')
set([ttx tty],'FontSize',fs)
...
set(cb,'fontsize',fs)
set(xcb,'string','topography (m)','fontsize',fs)
% Be very explicit
set(cb,'xtick',[-7500:3750:3750])
set(cb,'xticklabel',round(get(cb,'xtick')))
...
% As published in doi: 10.1117/12.892285
switch opt
  case 'SPIE2011'
   delete(t)
   delete(cb)
